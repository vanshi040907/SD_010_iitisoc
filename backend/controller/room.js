...
async function UserEnterRoom(req, res) {
  ...
  if (!room.hostpermission) {
    ...
    user.ActiveRoom = room._id;
    await user.save();
    const y = activeParticipants.some((p) => p._id.toString() === userid.toString())
    if (!y) {
...
module.exports = { handlecreateRoomId, UserEnterRoom, UserLeaveRoom, Name, Allowance, Denial, MyRooms };